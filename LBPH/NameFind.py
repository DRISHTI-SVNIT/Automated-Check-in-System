...
face = cv2.CascadeClassifier('C:/Users/Shubhi/Desktop/LBPH/haarcascade_frontal_face_default.xml')
glass_cas = cv2.CascadeClassifier('C:/Users/Shubhi/Desktop/LBPH/haarcascade_eye_tree_eyeglasses.xml')

WHITE = [255, 255, 255]


def FileRead():
    Info = open("C:/Users/Shubhi/Desktop/LBPH/Names.txt", "r")  # Open th text file in readmode
...
def ID2Name(ID, conf):
    if ID > 0:
        NameString = "Name: " + Names[ID - 1] + " Distance: " + (
...
def AddName():
    ...
    Info = open("C:/Users/Shubhi/Desktop/LBPH/Names.txt", "r+")
...
def DispID(x, y, w, h, NAME, Image):
    ...
    cv2.rectangle(Image, (Name_X_pos - 10, Name_y_pos - 25), (Name_X_pos + 10 + (len(NAME) * 7), Name_y_pos - 1),
                  (0, 0, 0), -2)  # Draw a Black Rectangle over the face frame
    cv2.rectangle(Image, (Name_X_pos - 10, Name_y_pos - 25), (Name_X_pos + 10 + (len(NAME) * 7), Name_y_pos - 1), WHITE,
                  1)
    cv2.putText(Image, NAME, (Name_X_pos, Name_y_pos - 10), cv2.FONT_HERSHEY_DUPLEX, .4,
...
def tell_time_passed():
    print
    'TIME PASSED ' + str(round(((time.clock() - now_time) / 60), 2)) + ' MINS'